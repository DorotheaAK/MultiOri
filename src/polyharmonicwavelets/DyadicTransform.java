...
public class DyadicTransform {
  ...
  private ComplexImage synthesisRedundant(ComplexImage[] array) {
    ...
    G.add(this.FS[2]);
    G.add(this.FS[3]);
    G.multiply(0.25D * this.sqrt2);
    H.multiply(0.25D);
    if (this.param.flavor == 7)
      G.divide(this.filters.ac); 
    int l = 1;
    for (int k = 1; k < this.J; ) {
      k++;
      l *= 2;
    } 
  ...
  private ComplexImage synthesisPyramid(ComplexImage[] array) {
    this.FS[1].multiply(0.5D);
    this.FS[2].multiply(0.5D);
    this.FS[3].multiply(0.5D);
    ComplexImage Gconj = this.FA[1].copyImage();
    Gconj.multiply(1.0D / this.sqrt2);
    ComplexImage[] G0 = this.FP;
    int l = 1;
    for (int i = 0; i < this.J; ) {
      i++;
      l *= 2;
    } 
    ...
    if (this.param.prefilter)
      Y1.divide(this.P); 
    Y1.iFFT2D();
    this.FS[1].multiply(2.0D);
    this.FS[2].multiply(2.0D);
    this.FS[3].multiply(2.0D);
    return Y1;
  }
  ...
  private ComplexImage displayBasis(ComplexImage image) {
    ...
    for (int j = 1; j <= this.J; j++) {
      ComplexImage complexImage = image.getSubimage(dx / 2, dx - 1, 0, dy / 2 - 1);
      complexImage.stretch();
      out.putSubimage(dx / 2, 0, complexImage);
      complexImage.getSubimageContent(dx / 2, dx / 2 + complexImage.nx - 1, dy / 2, dy / 2 + complexImage.ny - 1, image);
      complexImage.stretch();
      out.putSubimage(dx / 2, dy / 2, complexImage);
      complexImage.getSubimageContent(0, complexImage.nx - 1, dy / 2, dy / 2 + complexImage.ny - 1, image);
      complexImage.stretch();
      out.putSubimage(0, dy / 2, complexImage);
      dx /= 2;
      dy /= 2;
    } 
    ComplexImage sub = image.getSubimage(0, dx - 1, 0, dy - 1);
    sub.stretch();
