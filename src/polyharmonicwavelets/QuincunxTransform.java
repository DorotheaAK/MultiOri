...
public class QuincunxTransform {
  ...
  private ComplexImage[] analysisRedundant(ComplexImage image) {
    ...
    double sqrt2inv = 1.0D / this.sqrt2;
    ...
    array[this.J].iFFT2D();
    G.multiply(1.0D / this.sqrt2);
    GD.multiply(1.0D / this.sqrt2);
    H.multiply(1.0D / this.sqrt2);
    HD.multiply(1.0D / this.sqrt2);
    return array;
  }
  ...
  private ComplexImage synthesisRedundant(ComplexImage[] array) {
    ...
    H.multiply(1.0D / this.sqrt2);
    HD.multiply(1.0D / this.sqrt2);
    double sqrt2inv = 1.0D / Math.sqrt(2.0D);
    ComplexImage LP = array[this.J];
    LP.FFT2D();
    int l = 1;
    int j;
    for (j = 1; j < (this.J + 1) / 2; ) {
      j++;
      l *= 2;
    } 
    for (j = this.J; j > 0; j--) {
  ...
  private ComplexImage synthesisPyramid(ComplexImage[] array) {
    ...
    GlsD.conj();
    GlsD.divide(Gemodsqr, 1.0D, 0.0D);
    ComplexImage H = this.FS[0];
    ...
    LP.FFT2D();
    int l = 1;
    for (int i = 0, J12 = (this.J - 1) / 2; i < J12; ) {
      i++;
      l *= 2;
    } 
    int mj = this.J % 2;
  ...
  public ComplexImage displayBasis(ComplexImage image) {
    ...
    for (int j = 1; j <= this.J; j++) {
      if (j % 2 == 1) {
        ComplexImage complexImage = image.getSubimage(dx / 2, dx - 1, 0, dy - 1);
        complexImage.stretch();
        out.putSubimage(dx / 2, 0, complexImage);
        dx /= 2;
      } else {
        ComplexImage complexImage = image.getSubimage(0, dx - 1, dy / 2, dy - 1);
        complexImage.stretch();
