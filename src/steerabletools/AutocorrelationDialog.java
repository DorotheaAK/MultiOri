...
public class AutocorrelationDialog extends JDialog implements ActionListener, WindowListener, Runnable {
  ...
  public AutocorrelationDialog(ImagePlus imp, String title, Settings settings, WalkBar walk) {
    ...
    this.walk = walk;
    doDialog(imp);
    settings.record("Autocorrelation-spnOrder", (JSpinner)this.spnOrder, "3");
  ...
  private void doDialog(ImagePlus imp) {
    ...
    pn0.place(0, 1, (JComponent)this.spnOrder);
    ...
    panel.place(0, 0, (JComponent)pn0);
    panel.place(1, 0, (JComponent)this.walk);
  ...
  public void run() {
    ...
    if (this.job == this.bnGamma) {
      ...
      this.txtGamma.setText("" + (System.currentTimeMillis() - chrono) + " ms");
      ...
      this.txtIterative.setText("" + (System.currentTimeMillis() - chrono) + " ms");
